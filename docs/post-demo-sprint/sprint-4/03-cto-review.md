...
| **S4.2** | Calendar event types — only RIDE pills, or also café/race/training-camp pills? | **Multi-type from day one.** ~~`club_events.event_type` already exists in migration 0002.~~ **Correction (architect §G.2 caught it):** `event_type` is NOT in migration 0002 — confirmed absent from `0002_club_events.sql` and `schema.sql`. Migration 0006 (Phase 3) adds it with `DEFAULT 'ride'` so existing rows backfill cleanly. Use icons (🚴 ride, ☕ social, 🏁 race) — colour-coded would clash with the conflict pill amber. |
