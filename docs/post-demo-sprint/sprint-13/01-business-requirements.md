...
**v11.2.0 — My Account UI + #5 + housekeeping + cyclist-friendly copy sweep**
...
- **Cyclist-friendly copy sweep across v11.1.0 substrate surfaces** (founder feedback post-v11.1.0 ship 2026-05-03): replace "passphrase" with "password" everywhere users see it; extend lock/unlock metaphor (encrypt → lock; decrypt → unlock); component file names stay "passphrase" (dev-facing only)
- README sweep
...
#### AC-1.2.11 — Cyclist-friendly copy sweep (added post-v11.1.0 per founder feedback)

All user-facing copy on the v11.1.0 substrate surfaces is rewritten so cyclists, not developers, are the implied audience. Specifically:

| Surface | Before (v11.1.0) | After (v11.2.0) |
|---|---|---|
| `SetupPassphraseModal` Step 1 headline | "Set your passphrase" | "Set a password to lock your AI key" |
| `SetupPassphraseModal` Step 1 lede | "Your Anthropic key needs a passphrase. We use it to encrypt the key on your device — neither Cadence Club nor Cloudflare can read it. Forget the passphrase, lose the key (recovery code on the next step)." | "Pick a password — only you will know it. We use it to lock your Anthropic key on this device. Nobody at Cadence Club or Cloudflare can see your key, ever. Forget the password and you lose the key (we'll give you a backup code on the next screen)." |
| `SetupPassphraseModal` Step 1 field labels | "Passphrase" / "Confirm passphrase" | "Password" / "Confirm password" |
| `SetupPassphraseModal` Step 2 headline | "Save your recovery code" | "Save your backup code" |
| `SetupPassphraseModal` Step 2 lede | "Write this down. We can't recover it for you. This code resets your passphrase on a lost device — without it, you re-enter your Anthropic key from scratch." | "Write this down. If you forget your password, this code lets you start fresh. Without it, you'd have to enter your Anthropic key from scratch." |
| `SetupPassphraseModal` Step 2 CTA | "Encrypt my key" | "Lock my key" |
| `SetupPassphraseModal` Step 3 headline | "Encrypted" | "Locked. You're set." |
| `SetupPassphraseModal` Step 3 lede | "Your key is on your device, encrypted with your passphrase. Re-enter on each new device or use the recovery code." | "Your Anthropic key is locked on this device. On a new device, just enter your password again — or use your backup code if you forget it." |
| `PassphraseUnlockCard` eyebrow | "AI Coach · locked" | "AI Coach · locked" (unchanged — "locked" already non-tech) |
| `PassphraseUnlockCard` lede | "Enter your passphrase to use AI features in this session." | "Enter your password to use AI features." |
| `PassphraseUnlockCard` error | "That passphrase doesn't match. Try again or use your recovery code." | "That password doesn't match. Try again or use your backup code." |
| `PassphraseUnlockCard` link | "Forgot passphrase?" | "Forgot password?" |
| `MigrationBanner` lede | "Your Anthropic key currently lives in browser storage. Move it to encrypted storage so it persists across devices, encrypted with a passphrase only you know." | "Your Anthropic key is saved in this browser only. Lock it with a password so it works across your devices — only you will know the password." |
| `MigrationBanner` CTA | "Set up encryption" | "Add a password" |
| `/account/recover` headline | "Lost your passphrase?" | "Forgot your password?" |
| `/account/recover` lede | "Enter your recovery code. We'll clear the encrypted key on the server (it's unrecoverable without the old passphrase anyway). Set a new passphrase next, then re-enter your Anthropic key to encrypt against the new master key." | "Enter your backup code. We'll clear the locked key (we can't open it without the old password anyway). Then you'll set a new password and re-enter your Anthropic key." |
| `/account/recover` field placeholder | "XXXX-XXXX-XXXX-XXXX-XXXX-XXXX" | "Backup code: XXXX-XXXX-XXXX-XXXX-XXXX-XXXX" |
| `/account/recover` success body | "Your encrypted keys have been cleared. Visit your account to set a new passphrase and re-enter your Anthropic key." | "Your locked keys have been cleared. Visit your account to set a new password and re-enter your Anthropic key." |
| `cadence-club-recovery-code.txt` download body | "Cadence Club — recovery code" | "Cadence Club — backup code" |

**Vocabulary rules** (locked):
- "passphrase" → **"password"** in user-facing copy. Component file names + dev-facing comments stay "passphrase" (cryptographically correct).
- "encrypt"/"encryption" → **"lock"** in user-facing copy.
- "decrypt" → **"unlock"** in user-facing copy.
- "recovery code" → **"backup code"** in user-facing copy. (Internal naming `recovery_code_hash` / `/api/me/passphrase/recover` stays — those are dev surfaces.)
- "ciphertext" / "AAD" / "AES-GCM" / "PBKDF2" / "salt" / "iv" / "kdf_iterations" — never user-facing in any release.

**Note:** the database column `users.recovery_code_hash` and the worker endpoint path `/api/me/passphrase/recover` remain unchanged — those are non-user-visible surfaces. Renaming them would break the v11.1.0 deploy. Only the rendered HTML / button text / modal copy changes.

#### AC-1.2.12 — Tests pass after copy sweep
- All 283 existing tests still green (the copy change is presentational — no test references the user-facing strings).
- `tsc --noEmit` exit 0.
- `npm run build` green.
