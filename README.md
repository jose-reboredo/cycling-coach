...
**Live:** [cycling-coach.josem-reboredo.workers.dev](https://cycling-coach.josem-reboredo.workers.dev) · **Current release:** [v11.4.0](./CHANGELOG.md) · **Security policy:** [SECURITY.md](./SECURITY.md) · **Contributing:** [CONTRIBUTING.md](./CONTRIBUTING.md)
...
| `/how-it-works` | `how-it-works.tsx` | KPI explainer (v11.3.0) — CTL/ATL/TSB/TSS/streak/year-end forecast math + honest limits |
...
- **v11.4.0** — Sprint 14 follow-up / `/how-it-works` redesign + `FeatureSpread` molecule extraction. Per founder feedback ("the how-it-works page must have the same template as `/#what` — I like the pictures, easy to understand"), `/how-it-works` rebuilt with the same alternating picture/text layout used on the marketing landing. Each KPI section gets a visual: PmcStrip (CTL/ATL/TSB), an ATL decay chart (14 days, no training), a form scale (race-ready / fresh / building / overreaching ranges), a zone stack (TSS by zone × IF), weekly streak chips, a forecast bar, and a "what's not measured" honest-limits panel. Added `id="forecast"` anchor + the Today YTD card "How is this calculated?" link now deep-links to `/how-it-works#forecast` (was page-top before). Extracted `<FeatureSpread />` from Landing.tsx into `apps/web/src/components/FeatureSpread/` so both the marketing landing and `/how-it-works` consume the same molecule (per `feedback_atomic-design-extraction-from-bugs.md`). Removed a fabricated paragraph ("why no fixed 8 000 km target") that referenced a placeholder, not a real business decision.
- **v11.3.0** — Sprint 14 / Tester-readiness bundle. 18 of 21 founder-flagged tester-blocking issues fixed in a single bundled release. Headline fixes: TopBar "signed as" name now reads from `users.name` (database) with Strava as fallback; UserMenu z-index bumped from --z-dropdown to --z-overlay (was below sticky TopBar's stacking context, causing menu-clicks to register against the page beneath); "Edit Profile" menu item removed (duplicate of My Account); weekly streak counts ISO weeks with ≥ 1 training (was daily Strava activity); Personal section gender labels capitalized + country picker shows names not ISO codes + save errors surfaced (silent failure was the real "no success message" bug); new `/how-it-works` route explains CTL/ATL/TSB/TSS/streak/year-end forecast in cyclist language; Today YTD card replaces static "of 8000" goal with forward-looking forecast (linear YTD/days×365); Train tab section order is API-key-aware (key entry first when no key); Rides Coach Verdict toggle (open AND close); scroll-to-top on every dashboard sub-route change; logo redirect path-aware (/ for marketing, /dashboard/today for signed-in); tab-heading font aligned to v11.2.0 design-system display face. Investigations: RWGPS connect needs founder-side redirect URI verification; club-calendar personal-events report has no code defect (likely stale cache pre-v10.11.2 or scope confusion).
